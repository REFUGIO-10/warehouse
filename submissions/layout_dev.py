"""RAMA A / LAYOUT — iterate on create_layout() here.

Only touch create_layout(). The act() below is the proven greedy (verbatim from
submission.py, ~132 deliveries/seed) so your layouts produce real deliveries:

  python tools/check_submission.py submissions/layout_dev.py   # validates layout rules
  python tools/benchmark.py submissions/layout_dev.py --count 20

LAYOUT CONTRACT: 960 unique shelves in 1..50; not on a base entry cell; each shelf
needs >=1 adjacent EMPTY pickup cell; all walkable cells connected; deterministic.
OPTIMIZE: demand is uniform over your 960 shelves, robots come from all 4 borders.
Minimize average base<->shelf round trip while keeping aisles wide. Measure on the
multi-seed benchmark, never on 3 seeds.
"""
