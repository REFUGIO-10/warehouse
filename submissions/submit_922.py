...
def _flow(x, y, nx, ny):
    ...
    # vertical aisle column: the column right after each block (index BW in period)
    if x == nx and (x - LO) % PERIOD_X == BW:
        col_idx = (x - LO) // PERIOD_X
        if col_idx % 2 == 0 and ny > y: return FLOW_PENALTY   # prefer UP
        if col_idx % 2 == 1 and ny < y: return FLOW_PENALTY   # prefer DOWN
    # horizontal aisle row
    if y == ny and (y - LO) % PERIOD_Y == BH:
        row_idx = (y - LO) // PERIOD_Y
        if row_idx % 2 == 0 and nx > x: return FLOW_PENALTY    # prefer LEFT
        if row_idx % 2 == 1 and nx < x: return FLOW_PENALTY    # prefer RIGHT
    # perimeter highways
...
def _act(obs):
    brain=_BRAIN
    if brain.world is None: brain.world=_World(obs.grid)
    new_tick = brain.cur_tick is None or obs.tick!=brain.cur_tick
    if new_tick:
        if brain.cur_tick is None or obs.tick<brain.cur_tick: brain.reset_episode()
...
def _plan(brain, obs0):
    ...
    def priority(rid):
        ...
        return (0 if carrying else 1, remaining, -boost, rid)
