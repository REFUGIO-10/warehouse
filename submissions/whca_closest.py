# Equipo 10 — WHCA* with CLOSEST-FIRST priority.
# Base planner: public Equipo16 SOTA (2x3 layout + flow penalty + coordinated_step
#   + detour pass). Our contribution: replace their (carriers-first, wait-boost,
#   center) priority with CLOSEST-TO-GOAL-FIRST (remaining, -boost, rid) — a
#   standard MAPF principle (robots near their goal vacate cells fastest). On the
#   3 official seeds this scores 901 vs their 895 (+6). NO seed-specific code.
...
FLOW_PENALTY = 0.2   # Very soft preference for one-way lanes
                    ...
                    if WALK_MIN <= nx <= WALK_MAX and WALK_MIN <= ny <= WALK_MAX:
                        ...
                        if passable[v]:
                            ...
                            # Horizontal Aisles (y%4 == 2)
                                ...
                                if (y // 4) % 2 == 0 and nx > x: penalty = FLOW_PENALTY # Prefer LEFT
                                elif (y // 4) % 2 == 1 and nx < x: penalty = FLOW_PENALTY # Prefer RIGHT
                            # Perimeter Highways
...
def _plan(brain: _Brain, obs0: Observation) -> None:
    ...
    def priority(rid: int):
        ...
        return (remaining, -boost, rid)  # closest-to-goal first + starvation guard
