...
def _plan(brain: _Brain, obs0: Observation) -> None:
    ...
    final = _resolve_first_moves(brain, desired, order, goal_field)
...
def _astar(world, start, field, cell_res, edge_res):
    ...
    while open_heap:
        ...
        for action, m in nbrs[n]:
            ...
            # Apply soft flow penalty if moving against preferred direction
...
def _resolve_first_moves(brain, desired, order, goal_field=None):
    ...
    for rid in order:
        ...
        u = cur[rid]
        # Field-aware: prefer the free neighbor that makes the most progress
        # toward this robot's goal, instead of the first one in U/D/L/R order.
        field = goal_field.get(rid) if goal_field is not None else None
        cands = []
        for _, v in world.nbrs[u]:
            if v in occ:
                continue
            other_rid = by_cur.get(v)
            if other_rid is not None and final[other_rid] == u:  # would be a swap
                continue
            rank = int(field[v]) if field is not None else 0
            cands.append((rank, v))
        if cands:
            cands.sort()
            v = cands[0][1]
