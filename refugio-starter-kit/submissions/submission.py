"""REFUGIO submission — your starting point.

First dumb submission: proven valid layout + greedy pickup/drop. It scores
above zero so you can see deliveries in the replay and confirm the whole
pipeline works. Iterate by editing the two functions below.

  create_layout() -> your shelf layout (must follow the layout rules)
  act(observation) -> one Action for one robot at one tick
...
DELTA_BY_ACTION: dict[Action, Position] = {
    Action.RIGHT: (1, 0),
    Action.LEFT: (-1, 0),
    Action.DOWN: (0, 1),
    Action.UP: (0, -1),
...
def create_layout() -> dict[str, object]:
    # ponytail: reusing the kit's proven 960-shelf grid; replace once greedy plateaus.
...
def _can_move(observation: Observation, action: Action) -> bool:
    ...
    if not _is_empty(observation, target):
        return False
    return target not in set(observation.all_robot_positions.values())
